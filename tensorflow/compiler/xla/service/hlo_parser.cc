...
class HloParserImpl : public HloParser {
 ...
 private:
  ...
  // if `parse_computation_without_header` is true, the parsed text is sequence of 
  ...
  bool ParseHloModule(HloModule* module, bool parse_computation_without_header = false);
...
Status HloParserImpl::Run(HloModule* module) {
  ...
  if ((lexer_.GetKind() == TokKind::kw_HloModule) ||
    ...
    bool parse_computation_without_header = 
      (lexer_.GetKind() == TokKind::kw_HloModule) ? 
      false : true;
    if (!ParseHloModule(module, parse_computation_without_header)) {
...
bool HloParserImpl::ParseHloModule(HloModule* module, 
  ...
  if (!parse_computation_without_header) {
    if (lexer_.GetKind() != TokKind::kw_HloModule) {
